...
class SerialReader(QThread):
    ...
    def run(self):
        print('extracting favorite tweets ids...')
...
class SerialReader2(QThread):
    ...
    def run(self):
        data = []
        try:
            print('extracting recommended users...')
...
#         data = []
#         try:
#             data = extract_rec_users_tweets(self.q_uid, self.mcru_id)
#         except Exception as e:
#             print(e)
#         finally:
#             self.s.emit(data)
...
class ContactWidgetItem(QWidget):
    ...
    def __init__(self, name, image_url=None, color=None, width=40, height=40, parent=None):
...
class TwitterApp(QMainWindow):
    ...
    def update_flag2(self, data):
            ...
            for i in range(len(self.users)):
                contact_widget = ContactWidgetItem(str(self.users[i][0]), 'user-286.png', color='green')
                contact_widget.clicked.connect(self.click_contact)
                list_item = QListWidgetItem(self.contacts_list)
                list_item.setSizeHint(contact_widget.sizeHint())
                self.contacts_list.setItemWidget(list_item, contact_widget)
    ...
    def setup_ui(self):
        ...
        self.opt_name = ['origin', 'quoted', 'retweeted', 'replied', 'explore', 'Recommended Users']
        ...
        if self.contact == None:
            ...
            if self.opt == self.opt_name[5] and self.flag1 == 1:
                ...
                if self.opt == self.opt_name[0]:
                    tweets = extract_user_origin_tweets(self.favorites_tweets, self.q_uid)
                    message_number = len(tweets)
                if self.opt == self.opt_name[1]:
                    tweets = extract_user_quoted_tweets(self.favorites_tweets, self.q_uid)
                    message_number = len(tweets)
                if self.opt == self.opt_name[2]:
                    tweets = extract_user_retweeted_tweets(self.favorites_tweets, self.q_uid)
                    message_number = len(tweets)
                if self.opt == self.opt_name[3]:
                    tweets = extract_user_replied_tweets(self.favorites_tweets, self.q_uid)
                    message_number = len(tweets)
...
def extract_rec_users_tweets(q_uid, mcru_id):
    ...
    return tweets_rdd.filter(lambda x: json.loads(x)['user']['id'] in mcru_id or json.loads(x)['user']['id']==q_uid)\
                    .map(lambda x: dict(filter(lambda y: y[0] in necessary_keys, json.loads(x).items())))\
                    .collect()
...
if __name__ == "__main__":
    ...
    tweets_rdd = tweets_rdd.sample(False, 0.001, seed=42)
